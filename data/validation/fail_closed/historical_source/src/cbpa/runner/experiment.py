...
class CBPAExperiment(CBPALifecycle):
    ...
    def _execute_integrated(
        ...
        try:
            ...
            logger.warning("Integrated execution failed: %s — returning None", e)
            print(f"  [L4-INT] Integrated execution failed: {e}")
            return None
    ...
    def _certification_gate(self, candidates, pareto_result, cand_vr, cand_feas, contract, phase):
        """Choose the candidate to deploy from the Pareto ranking.

        Candidates are tried in ranking order (Pareto-selected first, then the
        remaining non-dominated and dominated feasible ones by V/R); the first
        whose Monte-Carlo feasibility reaches ``CERT_THRESHOLD`` is deployed.
        If none does, the Pareto-selected candidate is deployed on the
        deterministic check with guards armed and the deployment is flagged
        ``certified=False`` in the audit trail.  Returns ``(schedule, info)``.
        """
        ...
        for name in order:
            ...
            p = self.checker.check_stochastic(contract, mc, seed=self.mc_seed_base).p_feasible
    ...
    def _round_escalate(self, rc: RoundConfig, prior: list) -> PhaseResult:
        ...
        return self._phase4(p3.feasibility)
    ...
    def _phase1(self) -> PhaseResult:
        ...
        assert validation.is_valid, f"C1 validation failed: {validation.errors}"
        ...
        # If the simulation reveals a constraint violation, iterate through
        # Pareto-front candidates until a feasible one is found (closed-loop
        # CBPA verification).
        deployed = s1
        ...
        if sim_result is not None:
            ...
            if not report.is_feasible:
                print(f"\n  [L3] Simulation violated constraints: {report.violations}")
                print(f"  [L3] Re-planning: trying Pareto-front alternatives ...")
                for alt_name in pareto_result.non_dominated[1:]:
                    alt = next(
                        (c for c in candidates if c.name == alt_name), None
                    )
                    if alt is None:
                        continue
                    alt_sim = self._execute_simulation(alt, phase=1, contract=c1)
                    if alt_sim is None:
                        continue
                    alt_report = self.checker.check(c1, alt_sim.metrics)
                    if alt_report.is_feasible:
                        deployed = alt
                        metrics = alt_sim.metrics
                        vr = self.vr_scorer.compute(metrics, schedule_name=alt.name)
                        report = alt_report
                        print(f"  [L3] {alt.name} passes simulation verification")
                        break
                    else:
                        print(f"  [L3] {alt.name} also violates: {alt_report.violations}")
    ...
    def _phase2(self, p1: PhaseResult) -> PhaseResult:
        ...
        for drift in assumption_violations:
            ...
            logger.info(
                "Phase 2: Assumption A.%s violated: %s -> %s (%+.1f%%)",
    ...
    def _phase4(self, s2_report: FeasibilityReport) -> PhaseResult:
    ...
    def _phase5(self, p1: PhaseResult, p3: PhaseResult, p4: PhaseResult) -> PhaseResult:
        ...
        print(f"\n  [L5] Experience recorded to learning store "
              f"(total: {self.learning.total_records})")
        print(f"    Rejected: S2 | Accepted: S3 | Disturbance: demand +{self.config.demand_spike_pct}%")
        ...
        if sim_result is not None:
            ...
            if not report.is_feasible:
                ...
                for alt_name in pareto_result.non_dominated[1:]:
                    ...
                    if alt is None:
                        continue
                    alt_sim = self._execute_simulation(alt, phase=5, contract=c2)
                    if alt_sim is None:
                        continue
                    alt_report = self.checker.check(c2, alt_sim.metrics)
                    if alt_report.is_feasible:
                        deployed = alt
                        metrics = alt_sim.metrics
                        vr = self.vr_scorer.compute(metrics, schedule_name=alt.name)
    ...
    def _phase6_macro(self, p5: PhaseResult) -> PhaseResult:
        ...
        self.audit.record(
            ...
            sufficient=not self.adaptation.should_macro_adapt(
        ...
        best_p = max(micro_stoch.p_feasible, meso_stoch.p_feasible)
        ...
        # Fallback: macro was not needed (micro or meso was sufficient)
        logger.info(
            "Phase 6: Micro/meso adaptation sufficient — "
            "macro replan not triggered (p_feasible=%.2f%%)",
            best_p * 100,
        )
        return PhaseResult(
            phase=6,
            description=(
                f"R1 degradation handled by lower-tier adaptation "
                f"(p_feasible={best_p:.2%})"
            ),
        )
