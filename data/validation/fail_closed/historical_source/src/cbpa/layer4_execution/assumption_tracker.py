...
class AssumptionTracker:
    ...
    def update(self, name: str, current_value: float) -> AssumptionDrift | None:
        ...
        assumption.current_value = current_value

        expected = assumption.expected_value
        if expected == 0.0:
            drift_pct = 0.0
        else:
            drift_pct = (current_value - expected) / abs(expected) * 100.0

        assumption.drift_pct = round(drift_pct, 1)

        if abs(drift_pct) > assumption.tolerance_pct:
            assumption.violated = True
            sign = "+" if drift_pct >= 0 else ""
            attribution = (
                f"{name} drifted from {expected} to {current_value} "
                f"({sign}{drift_pct:.1f}%), exceeding {assumption.tolerance_pct}% tolerance"
            )
    ...
    def attribute_constraint_breach(
        ...
        for drift in self.drifts:
            ...
            if canonical in mapped_constraints:
                sign = "+" if drift.drift_pct >= 0 else ""
                results.append(
                    f"{drift.assumption.name} drift "
                    f"({sign}{drift.drift_pct:.1f}%) "
                    f"likely caused {constraint_name} ({constraint_type}) breach"
                )
        return results
