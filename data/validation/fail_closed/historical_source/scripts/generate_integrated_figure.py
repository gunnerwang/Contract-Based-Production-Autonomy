...
Creates a multi-panel figure showing:
  (a) Real-time KPI dashboard with constraint limits and phase annotations
...
_ap = _argparse.ArgumentParser(description="Integrated execution-stack figure (Figure 7)")
...
def panel_a_dashboard(ax_kpi, ax_timeline):
    """Panel (a): Real-time KPI dashboard with constraints (twin axis layout)."""
    ...
    ax_kpi.set_xticks(x)
    phase_labels = [f"Ph{p['phase']}\n{'OK' if p.get('feasible', True) else 'FAIL'}"
    ...
    ax_kpi.set_title("(a) Real-time KPI monitoring with constraint guards",
...
def panel_b_opcua(ax):
    ...
    ax.set_ylim(0, 10)
    ax.axis("off")
    ax.set_title("(c) OPC-UA node tree exposed to industrial clients",
