...
class Assumption(BaseModel):
    ...
    def check(self, observed: float) -> bool:
        ...
        ``False`` if violated.
        """
        values = (observed, self.expected_value, self.tolerance_pct)
        if any(not isinstance(v, (int, float)) or isinstance(v, bool) or not math.isfinite(v) for v in values) or self.tolerance_pct < 0:
            self.violated = True
            self.drift_pct = None
            raise ValueError(f"{self.name}: missing/non-finite assumption input or invalid tolerance")
        self.current_value = observed
        if self.expected_value == 0.0:
            # Relative change is undefined away from zero. A percentage
            # tolerance around zero has zero absolute width.
            self.violated = observed != 0.0
            self.drift_pct = None if self.violated else 0.0
        else:
            raw_drift = (observed - self.expected_value) / abs(self.expected_value) * 100.0
            self.drift_pct = round(raw_drift, 1)
            self.violated = abs(raw_drift) > self.tolerance_pct
...
def make_c3_factory() -> OutcomeContract:
    ...
    for a in c3.typed_assumptions:
        if a.name == "h2_availability":
            a.expected_value = 0.0   # absent in the next-shift contingency
