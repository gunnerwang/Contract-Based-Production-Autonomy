...
class StochasticFeasibilityReport(BaseModel):
    ...
    verification_scope: Literal["cell", "factory"] = "cell"
    checked_constraints: list[tuple[str, str, float]] = Field(default_factory=list)
    evaluation_errors: list[str] = Field(default_factory=list, description="Uncheckable constraints or missing/non-finite samples; any error prevents certification")
