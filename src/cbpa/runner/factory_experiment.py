...
class FactoryExperiment(CBPALifecycle):
    ...
    def _certification_gate(self, candidates, pareto_result, cand_metrics, cand_feas, contract, phase):
        ...
        seed = self.mc_seed_base + {1: 0, 3: 1, 5: 2, 6: 3}.get(phase, 0)
        ...
        for name in order:
            if name in seen or name not in by_name or not cand_feas[name].is_feasible:
                continue
            seen.add(name)
            mc = self.evaluator.evaluate_monte_carlo(by_name[name], n_samples=200, seed=seed)
            checked = self.checker.check_factory_stochastic(contract, mc, seed=seed)
    ...
    def _round_initial_deploy(self, rc: RoundConfig, prior: list) -> FactoryPhaseResult:
        ...
        c1 = self._prepare_contract(c1, 1, factory=True)
    ...
    def _round_monitor(self, rc: RoundConfig, prior: list) -> FactoryPhaseResult:
        ...
        # This analytical prototype checks the supplied phase-1 snapshot.
        # It is not a continuous factory sensor stream.
        if self._guard_enforcer is not None:
            events = self._guard_enforcer.check_and_enforce(metrics)
            self.audit.record(
                phase=2, layer="L4", action="runtime_snapshot_checked",
                contract_state="C1 active",
                guard_events=[event.model_dump() for event in events],
            )
    ...
    def _round_escalate(self, rc: RoundConfig, prior: list) -> FactoryPhaseResult:
        ...
        c2 = self._prepare_contract(c2, 4, factory=True, previous=prior[0].contract)
    ...
    def _round_adapt(self, rc: RoundConfig, prior: list) -> FactoryPhaseResult:
        ...
        c2 = self._prepare_contract(c2, 5, factory=True, previous=p1.contract)
    ...
    def _round_macro_adapt(self, rc: RoundConfig, prior: list) -> FactoryPhaseResult:
        ...
        c3 = make_c3_factory()
        c3 = self._prepare_contract(c3, 6, factory=True, previous=c2)
        ...
        self._require_deployment_certificate(cert, phase=6)
