...
from cbpa.models.metrics import StochasticFeasibilityReport
...
class OptimiserPlanner:
    ...
    def _certified(self, s: Schedule, contract: OutcomeContract) -> StochasticFeasibilityReport:
        mc = self.ev.evaluate_monte_carlo(s, n_samples=self.mc, seed=self.seed)
        return self.checker.check_stochastic(contract, mc, seed=self.seed)
    ...
    def plan(self, contract: OutcomeContract, demand: float, name: str = "OPT",
        ...
        for v, s in cand:
            if self.certify:
                checked = self._certified(s, contract)
                p = checked.p_feasible
                if checked.evaluation_errors or not p >= self.threshold:
                    continue
                p_best = p
            best, best_s = v, s
            break
        if best_s is not None and p_best is None:
            p_best = self._certified(best_s, contract).p_feasible
...
class FactoryOptimiserPlanner:
    ...
    def _certified(self, fs: FactorySchedule, contract: OutcomeContract) -> StochasticFeasibilityReport:
        mc = self.ev.evaluate_monte_carlo(fs, n_samples=self.mc, seed=self.seed)
        return self.checker.check_factory_stochastic(contract, mc, seed=self.seed)
    ...
    def plan(self, contract: OutcomeContract, routing, assignments, va, vb, da, db, name="FOPT",
        ...
        for v, pa, pb in cand:
            fs = self._build(name, pa, pb, routing, assignments, va, vb, da, db)
            if self.certify:
                checked = self._certified(fs, contract)
                p = checked.p_feasible
                if checked.evaluation_errors or not p >= self.threshold:
                    continue
                p_best = p
        ...
        if best_fs is not None and p_best is None:
            p_best = self._certified(best_fs, contract).p_feasible
