...
class ExperimentService:
    ...
    def __init__(self, config: ScenarioConfig | None = None, use_llm: bool = False):
        ...
        self._p4: PhaseResult | None = None
    ...
    def run_all(
        ...
        for phase_num in range(1, 6):
            ...
            if self._status.state == ExperimentState.ERROR:
                break
    ...
    def run_phase(self, phase_num: int, manager_decision: ManagerDecision | None = None) -> PhaseResult | None:
        ...
        """
        if self._status.state == ExperimentState.ERROR:
            return None  # A rejected lifecycle requires reset before another attempt.
        ...
        try:
            if phase_num == 1:
                pr = exp.run_single_phase(1, prior=self._status.completed_phases)
                self._p1 = pr

            elif phase_num == 2:
                assert self._p1 is not None, "Phase 1 must complete before Phase 2"
                pr = exp.run_single_phase(2, prior=self._status.completed_phases)

            elif phase_num == 3:
                assert self._p1 is not None and self._p1.contract is not None
                pr = exp.run_single_phase(3, prior=self._status.completed_phases)
                self._p3 = pr
                ...
                pr = exp.run_single_phase(5, prior=self._status.completed_phases)
    ...
    def resolve_escalation(self, decision: ManagerDecision) -> PhaseResult | None:
        ...
        try:
            ...
            logger.exception("Escalation completion failed")
            self._status.state = ExperimentState.ERROR
    ...
    def _complete_phase4(
        ...
        from cbpa.models.contract import make_c2
        assert self._p1 is not None and self._p1.contract is not None
        c2 = exp._prepare_contract(make_c2(), 4, previous=self._p1.contract)
        pr = PhaseResult(
            phase=4,
            contract=c2,
            decision=decision,
