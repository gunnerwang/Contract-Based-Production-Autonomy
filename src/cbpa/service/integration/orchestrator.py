...
class IntegrationOrchestrator:
    ...
    def deploy_and_run_schedule(
        ...
        Analytical fallback can populate display estimates when Isaac Sim is
        unreachable, but cannot satisfy guards requiring sensor observations.
        """
        ...
        for batch_idx in range(n_batches):
            ...
            sensor_valid = not sensor_data.get("stub") and not sensor_data.get("error")
            sensor_noise = acoustic.get("noise_db") if sensor_valid else None
            sensor_fatigue = human_op.get("fatigue_estimate") if sensor_valid else None
            ...
            if guard_enforcer is not None:
                # Only supplied observations can satisfy a runtime obligation.
                # Model estimates and rounded display values are not sensor
                # observations. Other KPIs are unavailable at this polling point.
                running_metrics = SimulationMetrics.model_construct(
                    noise_db=sensor_noise,
                    fatigue_index=sensor_fatigue,
                )
